...
6. **在 `Shell/service_account_binding.sh` 補上這個服務的綁定**

   在檔案中新增（與既有其他服務同樣的兩行一組模式）：

   ```bash
   gcloud iam service-accounts add-iam-policy-binding \
       --role="roles/iam.workloadIdentityUser" \
       --member="serviceAccount:static-map-242406.svc.id.goog[<service-namespace>-dev/<chart-name>]" \
...
7. **執行 Workload Identity 綁定**（操作者執行，黑名單腳本）
...
8. **若服務需要對外 HTTP 流量**：提供以下指令文字給操作者執行（不在 ArgoCD 同步範圍內，需手動 GCP 操作）：
...
```
Server=127.0.0.1;Port=5432;Database=<db>;Userid=n-c-b-a@static-map-242406.iam;Pooling=true;MinPoolSize=1;MaxPoolSize=100;ConnectionLifeTime=60;
```

使用者名稱固定為共用 GSA 的 IAM 帳號：`n-c-b-a@static-map-242406.iam`（不需密碼欄位）。
